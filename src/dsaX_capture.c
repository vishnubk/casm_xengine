...
int main (int argc, char *argv[]) {
  ...
  */
  ...
  while (1)
    ...
    got = recvfrom(udpdb.sock->fd,        // Socket file descriptor
               udpdb.sock->buf,        // Buffer to store received data
               UDP_PAYLOAD,            // Maximum length of buffer
               0,                      // Flags (none set)
               NULL,                   // Source address (not needed)
               NULL);                  // Address length (not needed)
    ...
    uint16_t aid = details.chan0/512 - 1;  // This will give values 0,1,2,3,4,5
	  ...
	  syslog(LOG_INFO, "Packet Details: timestamp=%lu, chan0=%u, board_id=%u, n_chans=%u, n_antpols=%u",
		 	  uint64_t aid, details.chan0, details.board_id, details.n_chans, details.n_antpols);
